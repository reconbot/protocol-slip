...
function _syncDecode(iterable: Iterable<Buffer>): IterableIterator<Buffer> {
  ...
  const returnPacket = () => {
    const packet = buffer.slice(0, writeCursor)
    buffer = buffer.slice(writeCursor)
    writeCursor = 0
    return { done: false, value: packet }
  }

  function next(): IteratorResult<Buffer> {
    if (done) {
      return { done, value: undefined as any }
    }
